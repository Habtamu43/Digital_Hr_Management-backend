...
export const HandleCreateRecruitment = async (req, res) => {
    try {
        ...
        const existingRecruitment = await Recruitment.findOne({
            where: { jobtitle, organizationID: req.ORGID }
        });
        ...
        return res.status(201).json({ success: true, message: "Recruitment created successfully", data: newRecruitment });
    } catch (error) {
        console.error(error);
...
export const HandleRecruitment = async (req, res) => {
    try {
        ...
        const recruitment = await Recruitment.findOne({
            where: { id: recruitmentID, organizationID: req.ORGID },
            include: [{ model: Applicant, as: "application" }]
        });
...
export const HandleUpdateRecruitment = async (req, res) => {
    try {
        ...
        // Handle adding new applications if provided
        if (applicationIDArray && Array.isArray(applicationIDArray)) {
            const existingApplications = recruitment.application.map(app => app.id);
            const selectedApplications = [];
            const rejectedApplications = [];

            applicationIDArray.forEach(appId => {
                if (!existingApplications.includes(appId)) selectedApplications.push(appId);
                else rejectedApplications.push(appId);
            });
            ...
            // Add new applicants
            await recruitment.addApplication(selectedApplications);
        }

        // Update other recruitment fields
        await recruitment.update({ jobtitle, description, departmentId: departmentID });
